...
TcpConnector::TcpConnector(boost::asio::io_context& io_context, const std::string& address, unsigned short port)
...
void TcpConnector::start_accept() {
    acceptor_.async_accept(
        [this](const boost::system::error_code& error, boost::asio::ip::tcp::socket socket) {
            if (!error) {
                auto new_connection = std::make_shared<ExternalConnection>(std::move(socket));
