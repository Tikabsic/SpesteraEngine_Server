...
UdpServer::UdpServer(boost::asio::io_context& io_context, const std::string& address, int port, ConnectionsManager* connmanager, MessageInterpreter* messageinterpreter)
	: socket_(io_context, udp::endpoint(boost::asio::ip::make_address(address), port)), conn_manager_(connmanager), message_interpreter_(messageinterpreter) {
	receive_data();
}
